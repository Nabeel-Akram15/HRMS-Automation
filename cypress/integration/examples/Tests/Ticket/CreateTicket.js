...
const ticketform = new CreateTicketForm()
const ticket = new InboxTicketDetails()
let ticketformdata
...
beforeEach(() => {
  ...
  ticket.createNewTicketButton().click()
})
...
describe('Ticket Form', function () {
  ...
  it('CC Field Search', function () {
    ticketform.ccDiv().within(() => {
      ticketform.ccFieldInput()
        .should('have.attr', 'aria-expanded', 'false')
      ticketform.ccDivePlaceHolder().click().type(ticketformdata.CC)
      cy.wait(300)
      ticketform.ccFieldAfterEnteringText()
  ...
  it('Category Field Search', function () {
    ticketform.categoryDiv().within(() => {
      ticketform.categoryFieldInput()
        .should('have.attr', 'aria-expanded', 'false')
      ticketform.categoryDivePlaceHolder().click().type(ticketformdata.Category)
      cy.wait(300)
      ticketform.categoryFieldAfterEnteringText()
  ...
  it('Priority Field Search', function () {
    ticketform.priorityDiv().within(() => {
      ticketform.priorityFieldInput()
        .should('have.attr', 'aria-expanded', 'false')
      ticketform.priorityDivePlaceHolder().click().type(ticketformdata.Priority)
      cy.wait(300)
      ticketform.priorityFieldAfterEnteringText()
  ...
  it('Submit form blank and check error messages', function () {
    ...
    ticketform.assigneError().should('be.visible')
      .and('have.css', 'color', 'rgb(244, 67, 54)')
      .and('have.text', 'Assignee is required')
    ticketform.titleError().should('be.visible')
      .and('have.css', 'color', 'rgb(255, 0, 0)')
      .and('have.text', 'Subject is required')
    ticketform.descriptionError().should('be.visible')
      .and('have.css', 'color', 'rgb(255, 0, 0)')
      .and('have.text', 'Description is required')
    ticketform.priorityError().should('be.visible')
      .and('have.css', 'color', 'rgb(244, 67, 54)')
